---
title: "R Notebook"
output: html_notebook fileURL <-"https://ed-public-download.apps.cloud.gov/downloads/Most-Recent-Cohorts-Scorecard-Elements.csv"
...
The final model isGRAD_DEBT_MDN_SUPP ~ GT_25K_P6 + C150_4_POOLED_SUPP + NPT4_PRIV + NPT4_PUB + NPT41_PRIV + NPT41_PUB + NPT42_PRIV + NPT43_PRIV + NPT44_PRIV + NPT44_PUB + NPT45_PRIV + NPT45_PUB + PCTFLOAN + PREDDEG + RELAFFIL + RET_FT4 + RET_FTL4 + RET_PT4 + RET_PTL4 + RPY_3YR_RT_SUPP + UGDS

#We will first remove some variables to fit the best model
```{r}
CSD$PCTPELL <- NULL
CSD$NPT42_PUB <- NULL
CSD$NPT43_PUB <- NULL
CSD$PPTUG_EF <- NULL
CSD$C150_L4_POOLED_SUPP<- NULL
CSD$MD_EARN_WNE_P10 <- NULL

```


# We will using decision dree to predicted median debt, fist, install the requisite packages
```{r}
...
```{r}
set.seed(1234)
train <- sample(nrow(CSD), 0.7*nrow(CSD)) # 70% of data are in the training set
CSD.train <- CSD[train,]
CSD.validate <- CSD[-train,]
dim(CSD.train)
dim(CSD.validate)
```
...
library(rpart)
set.seed(1234)
dtree <- rpart(GRAD_DEBT_MDN_SUPP~.,data=CSD.train,method="anova",
...
```{r}
library(rpart.plot)
prp(dtree.p,type=1,extra=100,
    fallen.leaves = T,main="Decison Tree")
```

# We can check how the tree doest vaildation test.
```{r}
dtree.pred <- predict(dtree.p,CSD.validate)
summary(dtree.pred)
dtree.pred
